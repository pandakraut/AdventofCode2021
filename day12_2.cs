...
namespace AdventOfCode2020
{
    class day12_2
    {
        ...
        public static void checkAllPaths(string endPoint, string currentPath, string smallTwice)
        {
            if (endPoint != "start")
            {
                if (mapping.ContainsKey(endPoint))
                {
                    if (endPoint == "end" && currentPath != "")
                    {
                        //invalid path;
                        return;
                    }
                    ...
                    {
                        checkAllPaths(nextPoint, currentPath, smallTwice);
                    }                    
                }
            }
            else
